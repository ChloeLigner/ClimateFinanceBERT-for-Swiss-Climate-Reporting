...
for chunk in csv_import(dir_path + "/Data/Alternatives/df_preprocessed_targetGroup.csv"):
    ...
    with torch.no_grad():
        ...
        text_seq, text_mask = tokenize(chunk['targetGroup'].tolist())
    ...
    print(chunk[['targetGroup', 'climate_relevance']])

    print('number of documents:', chunk.targetGroup.shape[0])
    print('number of relevant documents:', chunk.targetGroup[chunk.climate_relevance == 1].shape[0])
    
    # Mask all texts that are relevant
    chunk['climate_class_number'] = 500

    relevant_texts = chunk[chunk['climate_relevance'] == 1]
    valid_texts = relevant_texts['targetGroup'].dropna().tolist()  # Remove None values
    ...
    print(chunk[['targetGroup', 'climate_relevance', 'climate_class']])
...
df.to_csv(dir_path + "/Data/Alternatives/climate_finance_total_targetGroup.csv", encoding='utf8', index=False, header=True, sep='|')
