...
for chunk in csv_import(dir_path + "/Data/Alternatives/df_preprocessed_background.csv"):
    ...
    with torch.no_grad():
        ...
        text_seq, text_mask = tokenize(chunk['background'].tolist())
    ...
    print(chunk[['background', 'climate_relevance']])

    print('number of documents:', chunk.background.shape[0])
    print('number of relevant documents:', chunk.background[chunk.climate_relevance == 1].shape[0])
    
    # Mask all texts that are relevant
    chunk['climate_class_number'] = 500

    relevant_texts = chunk[chunk['climate_relevance'] == 1]
    valid_texts = relevant_texts['background'].dropna().tolist()  # Remove None values
    ...
    print(chunk[['background', 'climate_relevance', 'climate_class']])
...
df.to_csv(dir_path + "/Data/Alternatives/climate_finance_total_background.csv", encoding='utf8', index=False, header=True, sep='|')
