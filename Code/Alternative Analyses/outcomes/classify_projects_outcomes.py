...
for chunk in csv_import(dir_path + "/Data/Alternatives/df_preprocessed_outcomes.csv"):
    ...
    with torch.no_grad():
        ...
        text_seq, text_mask = tokenize(chunk['outcomes'].tolist())
    ...
    print(chunk[['outcomes', 'climate_relevance']])

    print('number of documents:', chunk.outcomes.shape[0])
    print('number of relevant documents:', chunk.outcomes[chunk.climate_relevance == 1].shape[0])
    
    # Mask all texts that are relevant
    chunk['climate_class_number'] = 500

    relevant_texts = chunk[chunk['climate_relevance'] == 1]
    valid_texts = relevant_texts['outcomes'].dropna().tolist()  # Remove None values
    ...
    print(chunk[['outcomes', 'climate_relevance', 'climate_class']])
...
df.to_csv(dir_path + "/Data/Alternatives/climate_finance_total_outcomes.csv", encoding='utf8', index=False, header=True, sep='|')
