...
for chunk in csv_import(dir_path + "/Data/Alternatives/df_preprocessed_lead.csv"):
    ...
    with torch.no_grad():
        ...
        text_seq, text_mask = tokenize(chunk['lead'].tolist())
    ...
    print(chunk[['lead', 'climate_relevance']])

    print('number of documents:', chunk.lead.shape[0])
    print('number of relevant documents:', chunk.lead[chunk.climate_relevance == 1].shape[0])
    
    # Mask all texts that are relevant
    chunk['climate_class_number'] = 500

    relevant_texts = chunk[chunk['climate_relevance'] == 1]
    valid_texts = relevant_texts['lead'].dropna().tolist()  # Remove None values
    ...
    print(chunk[['lead', 'climate_relevance', 'climate_class']])
...
df.to_csv(dir_path + "/Data/Alternatives/climate_finance_total_lead.csv", encoding='utf8', index=False, header=True, sep='|')
