...
for chunk in csv_import(dir_path + "/Data/df_translated.csv"):
    ...
    chunk['lead'][chunk.lead.isnull()] = ''
    chunk.Year = chunk.Year.astype(int)
    print(chunk.shape)

    if chunk.shape[0]>0:
        # preprocess the texts (lowercase, remove punctuations)
        chunk = preprocess_text.preprocess_text(chunk, 'lead')
        chunk['lead'] = chunk.text_preprocessed
...
df.to_csv(dir_path + "/Data/Alternatives/df_preprocessed_lead.csv", encoding='utf8', index=False, header=True, sep='|')
