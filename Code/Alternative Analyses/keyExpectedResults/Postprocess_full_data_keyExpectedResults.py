...
df = csv_import(dir_path + "/Data/Alternatives/climate_finance_total_keyExpectedResults.csv")

length_start = df.shape[0]
if length_start<5525:
    print(length_start)
    quit()
...
with open(dir_path + "/Code/ClimateFinanceBERT/Code/Multiclass/dictionary_classes.txt", "r") as f:
    label_map = json.load(f)

number_to_label = {v: k for k, v in label_map.items()}

df["climate_class"] = df["climate_class_number"].astype(int).map(number_to_label)
