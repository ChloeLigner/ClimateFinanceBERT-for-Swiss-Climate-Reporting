...
def csv_import(name, delimiter=";", chunksize=10 ** 5):
    x = pd.read_csv(name, encoding='utf-8', low_memory=False, delimiter=",", chunksize=chunksize,
...
for chunk in csv_import(dir_path + "/Data/df_translated.csv"):
    ...
    chunk['keyExpectedResults'][chunk.keyExpectedResults.isnull()] = ''
    chunk.Year = chunk.Year.astype(int)
    print(chunk.shape)

    if chunk.shape[0]>0:
        # preprocess the texts (lowercase, remove punctuations)
        chunk = preprocess_text.preprocess_text(chunk, 'keyExpectedResults')
        chunk['keyExpectedResults'] = chunk.text_preprocessed
...
df.to_csv(dir_path + "/Data/Alternatives/df_preprocessed_keyExpectedResults.csv", encoding='utf8', index=False, header=True, sep='|')
