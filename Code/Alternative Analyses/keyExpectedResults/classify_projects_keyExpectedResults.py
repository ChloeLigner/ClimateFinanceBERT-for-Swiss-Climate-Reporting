...
def load_dict(filename):
    ...
    return file
label_dict = load_dict(dir_path + "/Code/Multiclass/dictionary_classes.txt")
print(label_dict)
...
for chunk in csv_import(dir_path + "/Data/Alternatives/df_preprocessed_keyExpectedResults.csv"):
    ...
    with torch.no_grad():
        ...
        text_seq, text_mask = tokenize(chunk['keyExpectedResults'].tolist())
    ...
    print(chunk[['keyExpectedResults', 'climate_relevance']])

    print('number of documents:', chunk.keyExpectedResults.shape[0])
    print('number of relevant documents:', chunk.keyExpectedResults[chunk.climate_relevance == 1].shape[0])
    
    # Mask all texts that are relevant
    chunk['climate_class_number'] = 500

    relevant_texts = chunk[chunk['climate_relevance'] == 1]
    valid_texts = relevant_texts['keyExpectedResults'].dropna().tolist()  # Remove None values
    ...
    print(chunk[['keyExpectedResults', 'climate_relevance', 'climate_class']])
...
df.to_csv(dir_path + "/Data/Alternatives/climate_finance_total_keyExpectedResults.csv", encoding='utf8', index=False, header=True, sep='|')
